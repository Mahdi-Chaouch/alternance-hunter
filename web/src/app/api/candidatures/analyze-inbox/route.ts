...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl.replace(/\/$/, "")}/candidatures/analyze-inbox`, {
      method: "POST",
      cache: "no-store",
    ...
    console.error("[candidatures/analyze-inbox] Backend request failed:", message);
    return NextResponse.json(
      {
        detail: isTimeout
          ? "Le backend a mis trop de temps à répondre. Réessayez."
          : `Backend inaccessible: ${message}`,
      },
      { status: 503 },
    );
  }
